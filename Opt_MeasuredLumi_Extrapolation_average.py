...
for i in range(len(FillNumber16)):
     ...
     t_opt = lo.t_opt_eval(N_i, n_c, Xi, S_int, t_a16)
     print("t_opt=", t_opt,"Last time=", Times[len(Times)-1], "ta=", t_a16)
     ...
     if int(t_opt)>limit:
        ...
        if fit_result.params['b'].value <0 or fit_result.params['d'].value <0: 
            ...
            ax.plot([], [], 'kx ', label='Reduced Chi-Square ={:.5f}'.format(fit_result2.redchi))
            ...
            ax.plot([], [], 'kx ', label='Reduced Chi-Square ={:.5f}'.format(fit_result.redchi))    
        ...
        plt.savefig('ATLAS/OptimalFillsLuminosityEvolution2016/{}_Opt_Lumi_Evol_average.pdf'.format(text)) 
        #plt.show()
...
for i in range(len(FillNumber17)):
     ...
     #evaluate the optimal time for the current fill
     t_opt = lo.t_opt_eval(N_i, n_c, Xi, S_int, t_a17)
     print("t_opt=", t_opt,"Last time=", Times[len(Times)-1], "ta=", t_a17)
     ...
     if int(t_opt)>limit:
        ...
        ax.set_ylabel("Luminosity [cm^-2 s^-1]")
        ax.set_title('Luminosity evolution of optimal fill {}'.format(text))
        plt.savefig('ATLAS/OptimalFillsLuminosityEvolution2017/{}_Opt_Lumi_Evol_average.pdf'.format(text)) 
        #plt.show()
...
for i in range(len(FillNumber18)):
     ...
     #evaluate the optimal time for the current fill
     t_opt = lo.t_opt_eval(N_i, n_c, Xi, S_int, t_a18)
     print("t_opt=", t_opt,"Last time=", Times[len(Times)-1], "ta=", t_a18)
     ...
     if int(t_opt)>limit:
        ...
        try:   
            ...
            #performing fit of last segments of data
            fit_result=model.fit(L_fit, x=T_fit, a=1, b=0.2, c=1, d=0.2)
            Y = fit(X, fit_result.params['a'].value, fit_result.params['b'].value, fit_result.params['c'].value, fit_result.params['d'].value)
            print(fit_result.params['a'].value, fit_result.params['b'].value, fit_result.params['c'].value, fit_result.params['d'].value)
            #if the double exponential grows changing the fit model --> one exponential
            if fit_result.params['b'].value <0 or fit_result.params['d'].value <0: 
                print('Single exponential!')
                ...
                model2=Model(fit_lin)
                fit_result2=model2.fit(L_fit, x=T_fit, a2=1, b2=1)
                print(fit_result2.params['a2'].value, fit_result2.params['b2'].value)
                Y = fit_lin(X, fit_result2.params['a2'].value, fit_result2.params['b2'].value)
        k1=0   
        L=np.append(L_zero, Y)
        T=np.append(T_zero, X)
        Lumi_evol=np.array([])
        ...
        ax.set_ylabel("Luminosity [cm^-2 s^-1]")
        ax.set_title('Luminosity evolution of optimal fill {}'.format(text))
        plt.savefig('ATLAS/OptimalFillsLuminosityEvolution2018/{}_Opt_Lumi_Evol_average.pdf'.format(text)) 
        #plt.show()
